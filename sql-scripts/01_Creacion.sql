...
CREATE TABLE Usuario (
    ...
    Contraseña NVARCHAR(2000) NOT NULL,
...
CREATE TABLE Tool (
    ...
    CONSTRAINT CHK_Tool_Tipo CHECK (Tipo IN ('W', 'M', 'R', 'I', 'S')) -- Web, Matemática, Reporte, Integración, Soporte
);
...
CREATE TABLE Agente (
    ...
    Instruccion NVARCHAR(2000) NOT NULL,
    Tipo CHAR(1) NOT NULL,
    FechaCreacion DATETIME NOT NULL,
    FechaUltimaModificacion DATETIME NOT NULL,
    Accion BIT NOT NULL,
    FOREIGN KEY (IdUsuarioDueño) REFERENCES Usuario(IdUsuario),
    CONSTRAINT CHK_Agente_Tipo CHECK (Tipo IN ('S', 'A', 'R', 'M')), -- Soporte, Automatización, Reporte, Marketing
    CONSTRAINT CHK_Agente_Accion CHECK (Accion IN (0, 1))
);
...
CREATE TABLE Archivo (
    ...
    CONSTRAINT CHK_Archivo_Peso CHECK (Peso > 0)
);
